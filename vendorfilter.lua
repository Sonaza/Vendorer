...
function Addon:FilterItem(index)
	...
	for _, tokenData in pairs(tokens) do
		...
		if (not result and CanIMogIt) then
			...
			if (not negated) then
				result = result or
					(token == "unknowntransmog" and transmogable and not isKnownTransmog and not transmogForAnotherCharacter);
			else
				local negatedUnknown = token == "unknowntransmog" and transmogable and isKnownTransmog and
					not transmogForAnotherCharacter;
...
function VendorerFilteringButton_OnEnter()
	...
	if (Addon.db.global.UseTooltipSearch) then
		VendorerHintTooltip:AddLine("You can search by item name, rarity, type, slot, required currency or tooltip text.", nil
			, nil, nil, true);
	else
		VendorerHintTooltip:AddLine("You can search by item name, rarity, type, slot or required currency. Searching by tooltip text is currently disabled."
			, nil, nil, nil, true);
	end
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("You can also search for phrases by putting the words in quotes. The results will only include items with the words in the same order as the ones inside the quotes."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("Any and all filters can also be negated by prefixing the query word or phrase with |cffffffff! (an exclamation mark)|r."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("By prefixing a word or a phrase with |cffffffff+ (a plus)|r you can search for exact matches."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");

	VendorerHintTooltip:AddLine("|cffffffffMagic words|r");
	if (not CanIMogIt) then
		VendorerHintTooltip:AddLine("Predefined filters: |cffffffffusable, equippable, purchasable, unknown, available, canafford|r. Additional transmog filters exist if the dependency |cffffffffCan I Mog It|r is installed."
			, nil, nil, nil, true);
	else
		VendorerHintTooltip:AddLine("Predefined filters: |cffffffffusable, equippable, purchasable, unknown, available, canafford, transmogable, unknowntransmog|r."
			, nil, nil, nil, true);
	end
	...
	VendorerHintTooltip:AddLine("Prefix a number with the letter |cffffffffr|r. For example |cffffffffr|cfff3619492|r.", nil
		, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	...
	VendorerHintTooltip:AddLine("Enter a price value formatted like |cffffffff12|rg|cffffffff34|rs|cffffffff56|rc.", nil,
		nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("|cffffffffSearching for Ranges of Values|r");
	VendorerHintTooltip:AddLine("Search values can be prefixed with |cffffffff>, >=, <|r and |cffffffff<=|r to search for ranges of values."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("|cffffffffFiltering Examples|r");
	VendorerHintTooltip:AddLine("|cffffffff>=r|cfff3619490|r would find all items that require level higher than or equal to 90."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("|cffffffff>=|cfff36194250g|r |cffffffff<=|cfff36194500g|r would find all items that cost between 250 and 500 gold."
		, nil, nil, nil, true);
	VendorerHintTooltip:AddLine(" ");
	VendorerHintTooltip:AddLine("|cffffffff>=|cfff36194rare|r would find all items that are rare or better.", nil, nil, nil
		, true);
